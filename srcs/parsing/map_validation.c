...
/*   Updated: 2025/11/13 16:21:42 by thlibers         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (y < game->map.height)
	{
		...
		while (x < game->map.width && (y == 0 || y == game->map.height - 1))
		{
			if (game->map.grid[y][x] != WALL)
				return (ft_printf("invalid map : top/bottom border is not enclosed by walls\n"), 0);
			x++;
		}
		if (game->map.grid[y][0] != WALL || game->map.grid[y][game->map.width - 1] != WALL)
			return (ft_printf("invalid map : right/left border is not enclosed by walls\n"), 0);
		y++;
	}
	return (1);
}
...
int check_at_least_one(t_game *game)
{
	...
	if (game->map.collectibles < 1)
		return (ft_printf("Invalid map : at least one collectible required\n"), 0);
	if (game->map.exits != 1)
...
int	check_rectangular(t_game *game)
{
	...
	while (y < game->map.height)
	{
		if ((int)ft_strlen(game->map.grid[y]) != x)
			return (ft_printf("invalid map : the map must be rectangular\n"), 0);
		y++;
	}
	return (1);
}
